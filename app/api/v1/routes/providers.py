...
@router.get("/", response_model=List[ProviderResponse], dependencies=[Depends(deps.verify_roles([UserRole.ADMIN, UserRole.INVENTORY_MANAGER, UserRole.WORKER]))])
...
@router.post("/", response_model=ProviderResponse, status_code=status.HTTP_201_CREATED, dependencies=[Depends(deps.verify_roles([UserRole.ADMIN, UserRole.INVENTORY_MANAGER]))])
...
@router.put("/{provider_id}", response_model=ProviderResponse, dependencies=[Depends(deps.verify_roles([UserRole.ADMIN, UserRole.INVENTORY_MANAGER]))])
...
@router.delete("/{provider_id}", dependencies=[Depends(deps.verify_roles([UserRole.ADMIN]))])
